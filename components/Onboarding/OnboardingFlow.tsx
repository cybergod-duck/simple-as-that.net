            ...
            <div className="w-full max-w-2xl relative z-10 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]">
                ...
                {step === 'initial' && (
                    <div className="animate-fade-in space-y-6">
                        ...
                        <form onSubmit={handleInitialSubmit} className="space-y-4">
                            ...
                            <div className="group relative">
                                <label className={`block text-xs font-bold uppercase tracking-widest mb-3 transition-colors ${formData.hasWebsite
                                    ? (isDark ? 'text-purple-300 drop-shadow-[0_0_8px_rgba(168,85,247,0.4)]' : 'text-purple-600')
                                    : 'text-slate-500'
                                    }`}>Already Have a Website?</label>
                                <select
                                    required
                                    value={formData.hasWebsite}
                                    onChange={(e: any) => setFormData({ ...formData, hasWebsite: e.target.value, goals: '', existingUrl: '' })}
                                    className={`w-full bg-transparent border-b-2 py-4 text-xl font-bold focus:outline-none transition-colors cursor-pointer appearance-none rounded-none ${isDark
                                        ? 'border-purple-800 focus:border-purple-400 text-white [&>option]:bg-[#0d0521]'
                            ...
                            {formData.hasWebsite === 'yes' && (
                                <div className="group relative animate-fade-in">
                                    <label className={`block text-xs font-bold uppercase tracking-widest mb-3 transition-colors ${formData.existingUrl
                                        ? (isDark ? 'text-cyan-400 drop-shadow-[0_0_8px_rgba(0,255,255,0.4)]' : 'text-purple-600')
                                        : 'text-slate-500'
                            ...
                            {formData.hasWebsite && (
                                <div className="group relative animate-fade-in">
                                    ...
                                    >
                                        ...
                                        {formData.hasWebsite === 'yes' ? (
                                            <>
                                                <option value="redesign">Redesign My Website</option>
                                                <option value="rebuild">Rebuild From Scratch</option>
                                                <option value="add-features">Add New Features</option>
                                                <option value="leads">Get More Customers (Leads)</option>
                                            </>
                                        ) : (
                                            <>
                                                <option value="new-site">Build a New Website</option>
                                                <option value="new-business">Start a New Business</option>
                ...
                {step === 'thinking' && (
                    <div className="animate-fade-in flex flex-col items-center justify-center h-full text-center space-y-8">
                        <div className="relative flex items-center justify-center w-24 h-24">
                            <div className={`absolute inset-0 rounded-full border-2 border-t-transparent animate-spin ${isDark ? 'border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.3)]' : 'border-purple-500 shadow-[0_2px_10px_rgba(168,85,247,0.2)]'}`}></div>
                            <div className={`absolute inset-2 rounded-full border-2 border-b-transparent animate-[spin_1.5s_linear_infinite_reverse] ${isDark ? 'border-cyan-500/50 shadow-[0_0_15px_rgba(0,255,255,0.3)]' : 'border-cyan-500 shadow-[0_2px_10px_rgba(0,255,255,0.2)]'}`}></div>
                            <div className={`absolute inset-4 rounded-full border-2 border-l-transparent animate-[spin_2s_linear_infinite] ${isDark ? 'border-purple-400/50 shadow-[0_0_15px_rgba(168,85,247,0.3)]' : 'border-purple-400 shadow-[0_2px_10px_rgba(168,85,247,0.2)]'}`}></div>
                            <div className={`w-2 h-2 rounded-full animate-pulse transition-all ${isDark ? 'bg-white shadow-[0_0_10px_rgba(255,255,255,0.8)]' : 'bg-slate-900 shadow-[0_2px_4px_rgba(0,0,0,0.2)]'}`}></div>
                        </div>
                        <div className="space-y-2">
                            <h3 className={`text-2xl font-bold tracking-tight transition-colors ${isDark ? 'text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : 'text-slate-900'}`}>Building Your Preview</h3>
                ...
                {step === 'domain' && (
                    <div className="animate-fade-in space-y-12 w-full max-w-3xl mx-auto">
                        <div className="text-center md:text-left">
                            <h2 className={`text-4xl font-black tracking-tighter mb-4 transition-colors ${isDark ? 'text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]' : 'text-slate-900'}`}>Pick Your Domain.</h2>
                            <p className={`text-lg font-light tracking-wide transition-colors ${isDark ? 'text-cyan-100/60' : 'text-slate-500'}`}>
                        ...
                        {isCheckingDomain ? (
                            <div className="py-20 flex flex-col items-center justify-center space-y-6">
                                <div className={`w-12 h-12 border-4 border-t-transparent rounded-full animate-spin transition-colors ${isDark ? 'border-cyan-400' : 'border-cyan-500'}`}></div>
                                <span className={`font-mono text-xs tracking-widest uppercase transition-colors ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>Checking availability...</span>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                                <div className={`p-8 rounded-[2rem] border transition-all duration-300 hover:-translate-y-1 backdrop-blur-md ${isDark
                                    ? 'bg-black/50 border-cyan-500/20 hover:border-cyan-400/50'
                                    : 'bg-white/80 border-cyan-500/20 hover:border-cyan-500 shadow-sm'}`}>
                                    <h3 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>We'll Set It Up For You</h3>
                                    <p className="text-sm mb-6 text-slate-500">Pick a domain below and we handle everything.</p>
                                    <div className="space-y-3">
                                        {availableDomains.map((domain: any, i: number) => (
                                            <div key={i} className={`flex items-center justify-between p-4 rounded-xl border ${isDark ? 'bg-black/60 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                                                <div className="flex flex-col">
                                                    ...
                                                    {domain.available && <span className={`text-xs font-bold ${isDark ? 'text-green-400' : 'text-green-600'}`}>${domain.price}/yr</span>}
                                                </div>
                                                {domain.available ? (
                                                    <button onClick={() => handleDomainSelect(domain.domain)} className={`px-4 py-2 rounded-lg font-bold text-xs tracking-wider uppercase transition-colors border ${isDark
                                                        ? 'bg-white/10 text-white hover:bg-cyan-500/20 hover:text-cyan-400 border-white/10'
                                                        : 'bg-white text-slate-900 hover:bg-cyan-50 hover:text-cyan-700 border-slate-200 shadow-sm'}`}>Select</button>
                                                ) : (
                                ...
                                <div className="space-y-6 flex flex-col justify-between">
                                    <div className={`p-8 rounded-[2rem] border transition-all duration-300 hover:-translate-y-1 backdrop-blur-md ${isDark
                                        ? 'bg-black/50 border-orange-500/20 hover:border-orange-500/50'
                                        : 'bg-white/80 border-orange-400/30 hover:border-orange-500 shadow-sm'}`}>
                                        ...
                                        <button onClick={() => handleDomainSelect('cloudflare-pending')} className={`w-full py-3 border rounded-xl font-bold text-sm transition-colors ${isDark
                                            ...
                                            : 'border-orange-400 text-orange-600 hover:bg-orange-50 shadow-sm'}`}>Cloudflare Hub</button>
                                    </div>
                                    ...
                                    <div className={`p-8 rounded-[2rem] border transition-all duration-300 hover:-translate-y-1 backdrop-blur-md ${isDark
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button onClick={() => {
                                                ...
                                                : 'bg-slate-900 text-white hover:bg-slate-800 border-transparent shadow-md'}`}>Map</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
